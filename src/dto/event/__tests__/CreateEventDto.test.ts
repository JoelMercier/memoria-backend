...
describe('CreateEventDto', () => {
  const createValidPayload = () => ({
    ...
    eventCategorie: 'MONI', //-- [RÉPARÉ V4] Franconien d'acier au caractère près.
    eventSecteur: 'SYST',
    eventAction: 'DEMA',
    eventSeverite: 'WARN', //-- [RÉPARÉ V4] Unification sémantique de soute.
  ...
  it('accepte un payload valide et instancie correctement les types nominaux', () => {
    ...
    expect(l_oDto.idEvent.valeur).toBe(l_oPayload.idAppEvent); //-- [RÉPARÉ V4] Authentique accesseur .valeur Jojo-Style !
    expect(l_oDto.userId?.valeur).toBe(l_oPayload.userId); //-- [RÉPARÉ V4] Authentique accesseur .valeur Jojo-Style !
  ...
  it('applique automatiquement le repli de sévérité INFO si le champ est absent', () => {
    const l_oPayload = createValidPayload();
    delete (l_oPayload as any).eventSeverite; //-- [RÉPARÉ V4] Cible la bonne clé franconienne.
