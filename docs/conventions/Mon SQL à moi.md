# 🏺 Le manifeste du "Jojo-Style" : Normes et directives SQL pour Mémoria
**Description :** Référentiel absolu de rédaction des scripts de migration, d'ensemencement et de requêtage pour PostgreSQL (17+). Ce document sert de mémoire externe à l'intelligence artificielle en cas de rupture de contexte.
...
### L'ordre physique absolu des types :
...
*Note : Les zones de la clé primaire (PK) n'ont aucune obligation d'être en tête de fichier. Les index sont découplés du stockage en tas (Heap). MySQL, lui, en avait besoin.*
...
* **Mots-clés de 3 lettres et plus** : Toujours au format capitalisé (ex: `Create Table`, `Insert Into`, `Select`, `From`, `Where`, `Null`, `Not`, `Char`, `Bytea`, `Function`).
* **Mots-clés de moins de 3 lettres** : Toujours en minuscules strictes (ex: `on`, `to`, `is`, `or`, `in`, `as`).
...
* **Zéro alias par défaut** : Il est formellement interdit d'utiliser des alias de table (comme `FROM "Users" "us"`) dans les requêtes standards. Comme chaque zone est unique, PostgreSQL résout les noms sans aucune ambiguïté.
...
* **Nomenclature des clés** :
  ...
  * Clé étrangère (FK) : `préfixe` + `NomTableCible` + `Id` (ex: `"usRoleId"`, `"itContentTypeId"`).

---

## 📋 Rule 4 : La notation hongroise Memoria (Portées, types et improvisations)
...
* `f`  : Float (nombre à virgule flottante simple ou double)
* `d`  : Date (Date, date ou timestamp)
* `c`  : Char (caractère de longueur fixe, de 1 à 20)
* `b`  : Booléen
* `w`  : Word (mot, le plus souvent non signé)
...
### 3. Modificateurs et improvisations mimétiques
* `u`  : Unsigned (non signé)
* `p`  : Pointer (pointeur)
* `a`  : Array (tableau / flux d'octets bruts)
* `x`  : Hexadécimal (improvisation validée pour le type Bytea en paramètre : `p_ax`: array of hexa)
...
* **Alignement esthétique** : Pour le plaisir des yeux, le mot-clé `is` des commentaires doit être aligné verticalement sur l'ensemble du bloc.
